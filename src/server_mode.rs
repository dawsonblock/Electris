...
pub async fn start_server(
    ...
    // loudly warned so operators cannot silently misconfigure security.
    {
        ...
        if !sandbox_mandatory {
            // SAFETY: Only one thread runs at startup before the dispatcher
            // spawned, so there is no data race on the environment block.
            // ALLOW_NO_SANDBOX is never set in mandatory/strict mode.
            unsafe { std::env::set_var("ELECTRO_BROWSER_ALLOW_NO_SANDBOX", "1"); }
            tracing::warn!(
                sandbox = %sec.sandbox,
        ...
        // P2.9b — audit_log flag enforcement
        if !sec.audit_log {
            tracing::warn!(
        ...
        // Full cryptographic verification of signed skills is a longer-term
        // implementation. For now we emit a clear warning when the guard is absent.
        if sec.skill_signing != "required" {
            tracing::warn!(
                signing = %sec.skill_signing,
