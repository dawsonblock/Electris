...
async fn path_exists(path: &Path) -> Result<bool, ElectroError> {
    fs::try_exists(path).await.map_err(|e| {
        ElectroError::FileTransfer(format!("Failed to check path {}: {e}", path.display()))
    })
}

async fn atomic_write_bytes(path: &Path, data: &[u8]) -> Result<(), ElectroError> {
    ...
    if let Err(e) = fs::rename(&tmp_path, path).await {
        let _ = fs::remove_file(&tmp_path).await;
        return Err(ElectroError::FileTransfer(format!(
            "Failed to move temporary file {} into place at {}: {e}",
            tmp_path.display(),
            path.display()
        )));
    }
...
impl FileStore for LocalFileStore {
    async fn store(
        ...
        atomic_write_bytes(&full_path, &data).await?;
    ...
    async fn store_stream(
        ...
        atomic_write_bytes(&full_path, &buf).await?;
    ...
    async fn list(&self, prefix: &str) -> Result<Vec<String>, ElectroError> {
        ...
        if !path_exists(&search_dir).await? {
            return Ok(Vec::new());
        }
...
impl LocalFileStore {
    /// Recursively list all files under `dir` iteratively to avoid async recursion lifetime issues.
