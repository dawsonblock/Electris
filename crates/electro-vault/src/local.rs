...
impl LocalVault {
    ...
    async fn ensure_key(&self) -> Result<(), ElectroError> {
        if path_exists(&self.key_path).await? {
            return Ok(());
        }
        ...
        write_bytes_atomic(&self.key_path, key_bytes.as_ref()).await?;
    ...
    async fn load(&self) -> Result<(), ElectroError> {
        if !path_exists(&self.vault_path).await? {
            return Ok(());
        }
    ...
    async fn flush(&self) -> Result<(), ElectroError> {
        ...
        write_bytes_atomic(&self.vault_path, json.as_bytes()).await?;
...
impl Vault for LocalVault {
    async fn store_secret(&self, key: &str, plaintext: &[u8]) -> Result<(), ElectroError> {
        ...
        if key.trim().is_empty() {
            return Err(ElectroError::Vault("vault key must not be empty".into()));
        }
