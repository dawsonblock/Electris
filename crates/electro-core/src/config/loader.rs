...
{
    match toml::from_str::<T>(expanded) {
        Ok(config) => Ok(config),
        Err(toml_err) => match serde_yaml::from_str::<T>(expanded) {
            Ok(config) => Ok(config),
            Err(yaml_err) => Err(ElectroError::Config(format!(
                "Failed to parse {} as TOML ({}) or YAML ({})",
                source_label, toml_err, yaml_err
            ))),
        },
    }
}
...
fn config_paths() -> Vec<PathBuf> {
    vec![
        // 1. System config
        PathBuf::from("/etc/electro/config.toml"),
        // 2. User config
        paths::electro_home().join("config.toml"),
        // 3. Workspace config
        PathBuf::from("config.toml"),
        PathBuf::from("electro.toml"),
    ]
}
...
pub fn load_config(explicit_path: Option<&Path>) -> Result<ElectroConfig, ElectroError> {
    ...
    let source_label = if let Some(path) = explicit_path {
        config_content = read_config_file(path)?;
        path.display().to_string()
    } else {
        let mut source = "default configuration".to_string();
    ...
    parse_config_str::<ElectroConfig>(&expanded, &source_label)
}
...
pub fn load_agent_config(
    ...
    let source_label = if let Some(path) = explicit_path {
        ...
        let mut source = "agent configuration".to_string();
    ...
    let agent_config: AgentAccessibleConfig = parse_config_str(&expanded, &source_label)?;
...
"#;
    ...
    fn test_default_agent_config_path() {
        ...
        assert!(p.to_string_lossy().ends_with("agent.toml"));
    }
}
