...
fn db_timeout_error() -> ElectroError {
    ElectroError::Memory(format!(
...
fn current_unix_secs() -> u64 {
    match std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH) {
        ...
        Err(err) => {
            warn!(error = %err, "System clock is before UNIX_EPOCH; using 0 for timestamp");
            0
        }
    }
}

fn parse_lambda_tags(tags_json: &str, hash: &str) -> Vec<String> {
    match serde_json::from_str(tags_json) {
        Ok(tags) => tags,
        Err(err) => {
            warn!(hash = %hash, error = %err, "Failed to parse lambda memory tags; using empty tag list");
            Vec::new()
        }
    }
}
...
impl Memory for SqliteMemory {
    ...
    async fn lambda_store(&self, entry: LambdaMemoryEntry) -> Result<(), ElectroError> {
        let tags_json = serde_json::to_string(&entry.tags).map_err(ElectroError::Serialization)?;
...
fn lambda_row_to_entry(row: LambdaMemoryRow) -> LambdaMemoryEntry {
    let tags = parse_lambda_tags(&row.tags, &row.hash);
