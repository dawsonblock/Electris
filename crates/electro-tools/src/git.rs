...
impl Tool for GitTool {
    ...
    async fn execute(
        ...
        // ── Route through hardened shell runner ──────────────────────────
        // Use the same policy-controlled runner used by ShellTool so that
        ...
        match crate::shell::resolve_backend(&policy).await {
            Ok(crate::shell::ResolvedBackend::Docker) => {
                crate::shell::run_container_command(
                    crate::shell::ResolvedBackend::Docker,
                    &policy,
                    &parsed,
                    timeout_secs,
                    ctx,
            ...
            Ok(crate::shell::ResolvedBackend::Podman) => {
                crate::shell::run_container_command(
