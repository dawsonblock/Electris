...
impl BrowserTool {
    ...
    async fn ensure_browser(&self) -> Result<Page, ElectroError> {
        ...
        // --no-sandbox is gated on an explicit allowlist env-var. In production
        // the default is to NOT pass --no-sandbox (sandbox = "mandatory").
        // The startup code sets ELECTRO_BROWSER_ALLOW_NO_SANDBOX=1 only when
        // the operator has explicitly lowered security.sandbox to "permissive"
        // or "disabled" in the config.  Remote browser connections don't
        // launch a local Chrome, so the flag has no effect there.
        let allow_no_sandbox = crate::browser_runtime::env_flag("ELECTRO_BROWSER_ALLOW_NO_SANDBOX");

        builder = builder.arg("--disable-gpu");
        if allow_no_sandbox {
            tracing::warn!("Browser launched with --no-sandbox (ELECTRO_BROWSER_ALLOW_NO_SANDBOX=1 — security.sandbox is not mandatory)");
    ...
    async fn save_session(&self, page: &Page, session_name: &str) -> Result<String, ElectroError> {
        ...
        atomic_write_session_file(&path, &json)?;
...
fn atomic_write_session_file(path: &std::path::Path, contents: &str) -> Result<(), ElectroError> {
    ...
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(parent, perms).map_err(|e| {
            ElectroError::Tool(format!(
                "Failed to restrict session directory permissions for {}: {}",
                parent.display(),
                e
            ))
        })?;
    ...
    #[cfg(unix)]
    {
        ...
        if let Err(e) = std::fs::set_permissions(&tmp_path, perms) {
            ...
            return Err(ElectroError::Tool(format!(
                "Failed to restrict session temp file permissions: {}",
                e
            )));
        }
    }
    ...
    if let Err(e) = std::fs::rename(&tmp_path, path) {
        let _ = std::fs::remove_file(&tmp_path);
        return Err(ElectroError::Tool(format!(
            "Failed to move session temp file {} into place at {}: {}",
            tmp_path.display(),
            path.display(),
            e
        )));
...
fn sessions_dir() -> Result<std::path::PathBuf, ElectroError> {
    ...
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(&dir, perms).map_err(|e| {
            ElectroError::Tool(format!(
                "Failed to restrict sessions directory permissions for {}: {}",
                dir.display(),
                e
            ))
        })?;
    ...
    fn declarations(&self) -> CapabilityPolicy {
        CapabilityPolicy {
            ...
            browser_access: electro_core::policy::BrowserPolicy::Allowed {
                headless: true, // Default: run in headless mode
