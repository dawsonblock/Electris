...
impl TaskGraph {
        ...
        for id in &order {
            if !seen_ids.insert(id.clone()) {
                return Err(ElectroError::Internal(format!(
                    "Duplicate sub-task ID '{}' in task graph",
                    id
                )));
            }
        }
    ...
    pub fn mark_running(&mut self, id: &str) -> Result<(), ElectroError> {
        let (status, deps) = {
            ...
            (task.status.clone(), task.dependencies.clone())
        };

        match status {
            SubTaskStatus::Pending => {}
            SubTaskStatus::Running => return Ok(()),
            SubTaskStatus::Completed | SubTaskStatus::Failed => {
                return Err(ElectroError::Internal(format!(
                    "Cannot mark sub-task '{}' running from terminal state '{}'",
                    id, status
                )));
            }
        }
        ...
        for dep_id in &deps {
            let dep = self.tasks.get(dep_id).ok_or_else(|| {
                ElectroError::Internal(format!(
                    "Sub-task '{}' depends on missing sub-task '{}'",
                    id, dep_id
                ))
            })?;
            if dep.status != SubTaskStatus::Completed {
                return Err(ElectroError::Internal(format!(
                    "Cannot mark sub-task '{}' running before dependency '{}' completes",
                    id, dep_id
                )));
            }
        }
...
pub fn parse_decomposition(llm_response: &str, goal: &str) -> Result<Vec<SubTask>, ElectroError> {
    let mut parsed: Vec<(u32, String, Vec<u32>)> = Vec::new();
    ...
    for line in llm_response.lines() {
        ...
        if let Some((number, description, deps)) = parse_numbered_line(trimmed) {
            parsed.push((number, description, deps));
        }
    }

    if parsed.is_empty() {
        warn!("Failed to parse LLM decomposition — falling back to single task");
        return Ok(vec![SubTask::new("1", goal)]);
    }

    let mut step_map: BTreeMap<u32, usize> = BTreeMap::new();
    let mut duplicate_numbers = false;
    for (idx, (number, _, _)) in parsed.iter().enumerate() {
        if step_map.insert(*number, idx).is_some() {
            duplicate_numbers = true;
        }
    }

    if duplicate_numbers {
        warn!("Duplicate step numbers in decomposition — renumbering sequentially");
    }

    let mut subtasks: Vec<SubTask> = Vec::with_capacity(parsed.len());
    for (idx, (number, description, deps)) in parsed.iter().enumerate() {
        let final_id = (idx + 1).to_string();
        let mut task = SubTask::new(final_id.clone(), description.clone());

        let mut normalized_deps: BTreeSet<String> = BTreeSet::new();
        for dep in deps {
            if let Some(source_idx) = step_map.get(dep) {
                if *source_idx < idx {
                    normalized_deps.insert((source_idx + 1).to_string());
                }
            } else if !duplicate_numbers && *dep > 0 {
                let dep_index = (*dep as usize).saturating_sub(1);
                if dep_index < idx {
                    normalized_deps.insert(dep.to_string());
                }
            }
        }

        if task.description.trim().is_empty() {
            task.description = format!("Step {} from goal: {}", number, goal);
        }
        task.dependencies = normalized_deps.into_iter().collect();
        subtasks.push(task);
    }
